...
class PostsController {
    ...
    async getPosts (req: Request<{}, PostPaginationOutputType, DefaultQueryParamsInputType>, res: Response<PostPaginationOutputType>) {
        try {
            ...
            const posts: PostPaginationOutputType = await this.postMongoQueryRepository.findAllForOutput(sanitizedQuery)
    ...
    async getPost (req: Request<IdParamInputType, PostOutputType>, res: Response<PostOutputType>) {
        try {
            const post: PostOutputType | null = await this.postMongoQueryRepository.findForOutputById(req.params.id)
    ...
    async createPost (req: Request<{}, PostOutputType, PostBodyInputType>, res: Response<PostOutputType | string>) {
        try {
            const result: Result<string | null> = await this.postService.createPost(req.body)
    ...
    async updatePost (req: Request<IdParamInputType, PostOutputType, PostBodyInputType>, res: Response<PostOutputType>) {
        try {
            const result: Result<boolean | null> = await this.postService.updatePost(req.params.id, req.body)
    ...
    async deletePost (req: Request<IdParamInputType>, res: Response) {
        try {
            const result: Result<boolean | null> = await this.postService.deletePost(req.params.id)
            if (result.status === ResultStatus.NotFound) {
                res.status(HTTP_CODES.NOT_FOUND).send()
                return
            }
