...
class UserMongoQueryRepository {
    ...
    async findDetailedUserById(id: string): Promise<DetailedUserOutputType | null> {
        const user: User | null = await UserModel.findOne({_id: new ObjectId(id)})
    ...
    mapToDetailedUser({_id, login, email, createdAt, ...rest}: User): DetailedUserOutputType {
        return {
            id: _id.toString(),
            login,
            email,
    ...
    mapToAuthenticatedUser({_id, password, emailConfirmation, createdAt, ...rest}: User): AuthenticatedUserOutputType {
        return {
            ...rest,
            userId: _id.toString(),
        }
